...
import (
	...
	"github.com/jknetwork/jknetwork/v3/eth/fetcher"
...
// NewProtocolManager returns a new ethereum sub protocol manager. The JKNetwork sub protocol manages peers capable
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing JKNetwork peer", "peer", id)

	// Unregister the peer from the downloader and JKNetwork peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping JKNetwork protocol")
	...
	log.Info("JKNetwork protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("JKNetwork peer connected", "name", p.Name())

	// Execute the JKNetwork handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("JKNetwork handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("JKNetwork peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Error("JKNetwork message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the JKNetwork sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // JKNetwork network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
