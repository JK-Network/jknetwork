...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", JKNetwork.LightScryptN, JKNetwork.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.jknetwork", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	// Generate the mobile bindings for JKNetwork and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "io.jknetwork", "github.com/jknetwork/jknetwork/v3/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "io", "jknetwork", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
