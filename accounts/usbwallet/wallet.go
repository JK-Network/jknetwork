...
type wallet struct {
	...
	deriveChain     jknetwork.ChainStateReader  // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain jknetwork.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
